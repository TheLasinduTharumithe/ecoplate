...
export default function RegisterPage() {
    ...
    const [role, setRole] = React.useState<Role>("restaurant");
    ...
    React.useEffect(() => {
        ...
        const ctx = gsap.context(() => {
            gsap.set([".intro-chip", ".hero-title", ".hero-copy", ".hero-stat", ".register-card"], {
                opacity: 0,
                y: 24,
            });

            const tl = gsap.timeline({ defaults: { ease: "power3.out" } });
            tl.to(".intro-chip", { opacity: 1, y: 0, duration: 0.55 })
                .to(".hero-title", { opacity: 1, y: 0, duration: 0.8 }, "-=0.28")
                .to(".hero-copy", { opacity: 1, y: 0, duration: 0.72 }, "-=0.42")
                .to(".hero-stat", { opacity: 1, y: 0, duration: 0.5, stagger: 0.08 }, "-=0.3")
                .to(".register-card", { opacity: 1, y: 0, duration: 0.72 }, "-=0.42");
    ...
    return (
        ...
        >
            ...
            <div className="pointer-events-none absolute inset-0 z-0 bg-gradient-to-br from-black/35 via-[#04110d]/70 to-black/55" />

            {/* floating ambient blobs */}
            <div className="floating-orb-1 pointer-events-none absolute left-[7%] top-[10%] z-0 h-44 w-44 rounded-full bg-emerald-400/12 blur-3xl" />
            <div className="floating-orb-2 pointer-events-none absolute bottom-[8%] right-[10%] z-0 h-52 w-52 rounded-full bg-lime-300/10 blur-3xl" />
            <div className="floating-orb-3 pointer-events-none absolute right-[34%] top-[16%] z-0 h-32 w-32 rounded-full bg-emerald-300/8 blur-3xl" />
            ...
            <div className="relative z-10 mx-auto flex min-h-screen w-full max-w-7xl items-center px-6 py-10 lg:px-8">
                ...
                {" "}EcoPlate network
                        ...
                        <p className="hero-copy mt-6 max-w-xl text-base leading-8 text-white/68">
                            Create a premium EcoPlate account for your restaurant or charity and
                            start participating in a more intelligent, sustainable food donation ecosystem.
                        </p>
                            ...
                            {[
                                {
                                    ...
                                    value: "Restaurants",
                                    label: "Post surplus food quickly",
                                },
                                {
                                    icon: <HeartHandshake className="h-4 w-4 text-emerald-300" />,
                                    value: "Charities",
                                    label: "Claim support efficiently",
                                },
                                {
                                    icon: <ArrowRight className="h-4 w-4 text-emerald-300" />,
                                    value: "Real-Time",
                                    label: "Smarter coordination flow",
                                },
                            ].map((item) => (
                                <div
                                    key={item.value}
                    ...
                    <section className="mx-auto w-full max-w-xl lg:max-w-none">
                        <div
                            ref={cardRef}
                            className="register-card relative overflow-hidden rounded-[34px] border border-white/10 bg-white/[0.08] p-6 shadow-[0_24px_90px_rgba(0,0,0,0.42)] backdrop-blur-2xl sm:p-8 will-change-transform"
                                ...
                                <div className="mb-6 flex items-center gap-4">
                                    ...
                                    <div>
                                        ...
                                        <p className="mt-1 text-sm text-white/52">
                                            EcoPlate partner registration
                                        </p>
                                    </div>
                                </div>

                                <div className="mb-5 grid grid-cols-2 gap-3 rounded-[24px] border border-white/10 bg-black/15 p-2">
                                    {(["restaurant", "charity"] as Role[]).map((r) => (
                                        <button
                                            key={r}
                                            type="button"
                                            onClick={() => setRole(r)}
                                            className={cx(
                                                "rounded-2xl px-4 py-3 text-sm font-semibold capitalize transition",
                                                role === r
                                                    ? "bg-gradient-to-r from-emerald-400 to-lime-300 text-[#062116] shadow-[0_8px_24px_rgba(52,211,153,0.22)]"
                                ...
                                <form onSubmit={handleRegister} className="space-y-4">
                                    <div className="flex items-center gap-4">
                                        <button
                                            ...
                                            className="group relative h-16 w-16 shrink-0 overflow-hidden rounded-2xl border border-white/10 bg-white/6 transition hover:bg-white/10"
                                        >
                                            {avatar ? (
                                                <img
                                                    src={avatar}
                                                ...
                                                <div className="flex h-full w-full items-center justify-center">
                                                    <UserRound className="h-6 w-6 text-white/45" />
                                                </div>
                                            )}
                                        ...
                                        <div className="flex-1">
                                            <label className={labelCls}>
                                                {role === "restaurant" ? "Restaurant Name" : "Organization Name"}
                                            </label>
                                            <input
                                                type="text"
                                                value={name}
                                    ...
                                    >
                                        {loading ? (
                                            ...
                                            <>
                                                {`Create ${role === "restaurant" ? "Restaurant" : "Charity"} Account`}
                    ...
                    or continue
                                ...
                                <p className="mt-4 text-center text-[11px] leading-5 text-white/32">
                                    Designed for trusted onboarding across restaurants and charities.
                                </p>
                            </div>
                        </div>
                    </section>
